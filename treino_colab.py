...
# -*- coding: utf-8 -*-
# ============================================================================
# RIGELSLM - TREINO CONTÍNUO E INTELIGENTE (v1.2.0) - OTIMIZADO PARA COLAB
# Data: 06/08/2026 | Arquivos de treino: 1.089
# ============================================================================
# CORREÇÕES/OTIMIZAÇÕES v1.2.0:
# - CORRIGIDO: multiprocessing_context só é usado se num_workers > 0
# - Aumento de threads e paralelismo (torch.set_num_threads, float32 matmul)
# - Pré-carregamento opcional dos tokens em memória (--preload)
# - Suporte a torch.compile (--compile)
# - Gradient checkpointing opcional (--checkpointing)
# - Ajuste automático de batch_size para GPU T4
# - DataLoader com persistent_workers para reduzir overhead
# - Correção na listagem de pastas para evitar pastas vazias
# ============================================================================
...
# Parâmetros otimizados para Colab T4 (ajustáveis via argumentos)
BATCH_SIZE = 16                     # aumentado de 8 para 16 (T4 suporta)
GRADIENT_ACCUMULATION = 4           # efetivo 64
EPOCHS = 20
LEARNING_RATE = 3e-4
WEIGHT_DECAY = 0.01
WARMUP_STEPS = 1000
...
# ============================================================================
# ⚙️ TREINO_COLAB — esta CÓPIA treina SOMENTE arquivos .txt destas pastas.
#    As pastas abaixo devem estar DENTRO da pasta passada via --dados.
#    Se nenhuma for encontrada, usa a pasta inteira (ainda assim só .txt).
# ============================================================================
PASTAS_TREINO = [
    "datasets3_89", "datasets3_90", "datasets3_91", "datasets3_92",
    "datasets3_93", "datasets3_94", "datasets3_95", "datasets3_96",
    "datasets3_97", "datasets3_98", "datasets3_99", "datasets3_100",
    "datasets3_101", "datasets3_102",
    "processed_limpo1", "processed_limpo2", "processed_limpo3",
    "processed_limpo4", "processed_limpo5",
    "PerguntaseRespostas",
    "completos",
]

def _resolver_pastas_treino(base: str) -> List[str]:
    """Retorna as pastas de PASTAS_TREINO que existem dentro de 'base' e que contêm arquivos .txt.
    Se NENHUMA existir, retorna [base] (usa a pasta inteira como fallback)."""
    achadas = []
    for nome in PASTAS_TREINO:
        p = os.path.join(base, nome)
        if os.path.isdir(p):
            # Verifica se há pelo menos um .txt (evita pastas vazias)
            tem_txt = False
            for raiz, _, arquivos in os.walk(p):
                if any(a.endswith('.txt') for a in arquivos):
                    tem_txt = True
                    break
            if tem_txt:
                achadas.append(p)
...
def listar_subpastas_com_contagem(pasta_base: str, usar_registro: bool = False) -> List[Tuple[str, str, int]]:
    ...
    subpastas = []
    extensoes = ('.txt',)  # SÓ .TXT nesta cópia (treino_colab)
...
def listar_arquivos_recurssivo(pasta: str) -> List[str]:
    ...
    if not os.path.exists(pasta):
        return arquivos
    for base in _resolver_pastas_treino(pasta):
...
class RigelSLM(nn.Module):
    def __init__(self, use_checkpointing: bool = False):
        ...
        if use_checkpointing:
            decoder_layer.gradient_checkpointing = True  # Ativa checkpointing na camada
...
# ============================================================================
# 6. DATASET STREAMING COM PRÉ-CARREGAMENTO OPCIONAL
# ============================================================================
...
class StreamingTextDataset(IterableDataset):
    def __init__(self, pasta_dados: str, tokenizer: Tokenizer, seq_len: int = SEQ_LEN,
        ...
        self._cache = None  # cache de sequências tokenizadas (se preload=True)
        self._arquivos = None
    ...
    def _build_cache(self):
        """Pré-carrega e tokeniza todos os arquivos em memória (se preload=True)."""
        if self._cache is not None:
            return
        log(f"🔄 Construindo cache para {len(self._listar_arquivos())} arquivos...", "INFO")
        ...
        for caminho in self._listar_arquivos():
            try:
                ...
                for i in range(0, max(1, len(seq) - self.seq_len), stride):
                    ...
                    cache.append(torch.tensor(chunk, dtype=torch.long))
                contador += 1
                if contador % 100 == 0:
                    log(f"   Cache: {len(cache)} sequências processadas", "INFO")
            except Exception as e:
        ...
        self._cache = cache
        log(f"✅ Cache finalizado: {len(cache)} sequências em memória.", "INFO")

    def __iter__(self):
        if self.preload:
            self._build_cache()
            for seq in self._cache:
                yield seq
        else:
            # Modo streaming original (abre arquivos um a um)
            bos_id = self.tokenizer.token_to_id(BOS_TOKEN)
            eos_id = self.tokenizer.token_to_id(EOS_TOKEN)
            sep_id = self.tokenizer.token_to_id(SEP_TOKEN)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Treino do RigelSLM com aprendizado contínuo (otimizado para Colab)")
    ...
    parser.add_argument("--seq-len", type=int, default=SEQ_LEN,
                        help=f"Comprimento da sequência (padrão: {SEQ_LEN})")
    ...
    parser.add_argument("--max-arquivos", type=int, default=None,
                        help="Limite de arquivos (se não definido, pergunta interativamente)")
    parser.add_argument("--val-batches", type=int, default=VAL_BATCHES_LIMIT,
                        help="Número de batches para validação")
    parser.add_argument("--incluir-ultrachat", action="store_true",
                        help="Inclui pastas ultrachat (não usado nesta versão)")
    parser.add_argument("--test", type=str, default=None,
                        help="Modo teste (avalia modelo em uma pasta)")
    parser.add_argument("--num-workers", type=int, default=None,
                        help="Número de workers (recomendado: 2-4 para Colab)")
    parser.add_argument("--save-every", type=int, default=50,
                        help="Salvar checkpoint a cada N batches")
    parser.add_argument("--precision", type=str, default="amp",
                        choices=["fp32", "fp16", "amp"],
                        help="Precisão (amp recomendado para GPU)")
    parser.add_argument("--early-stop-patience", type=int, default=PATIENCE,
                        help="Paciência para early stopping")
    parser.add_argument("--no-interactive", action="store_true",
                        help="Pula menu interativo")
    parser.add_argument("--epochs", type=int, default=None,
                        help="Número total de épocas (sobrescreve EPOCHS)")
    parser.add_argument("--usar-registro", action="store_true",
                        help="Usa registro persistente (registro_pastas.json)")
    ...
    parser.add_argument("--preload", action="store_true",
                        help="Pré-carrega todos os tokens em memória (acelera, mas usa RAM)")
    parser.add_argument("--compile", action="store_true",
                        help="Aplica torch.compile ao modelo (requer PyTorch 2.0+)")
    parser.add_argument("--checkpointing", action="store_true",
                        help="Ativa gradient checkpointing no decoder (reduz memória)")
    parser.add_argument("--threads", type=int, default=0,
                        help="Número de threads para CPU (0 = auto)")

    args = parser.parse_args()

    # ====== OTIMIZAÇÕES DE PERFORMANCE (início) ======
    # Aumenta o número de threads para operações CPU-bound (tokenização, etc.)
    if args.threads == 0:
        # Auto-detecção: usa até 8 threads, mas respeita os núcleos disponíveis
        cpu_count = os.cpu_count() or 1
        threads = min(8, cpu_count)
    else:
        threads = args.threads
    torch.set_num_threads(threads)
    # Configura a precisão da multiplicação de matrizes para alta performance
    torch.set_float32_matmul_precision('high')
    print(f"🔧 Threads configuradas: {threads} | float32 matmul precision: high")
    # =================================================
    ...
    # --- RESUMO DAS PASTAS DE TREINO (somente .txt) ---
    pastas_resolvidas = _resolver_pastas_treino(PASTA_DADOS)
    if len(pastas_resolvidas) == 1 and pastas_resolvidas[0] == PASTA_DADOS:
        log(f"⚠️ Nenhuma das {len(PASTAS_TREINO)} pastas da lista foi achada dentro de {PASTA_DADOS}. Usando a pasta inteira (só .txt).", "WARNING")
    else:
    ...
    # --- PERGUNTA QUANTOS ARQUIVOS USAR ---
    ...
    if args.test:
        ...
        model = RigelSLM(use_checkpointing=False).to(DISPOSITIVO)
        ...
        test_dataset = StreamingTextDataset(
            ...
            max_arquivos=MAX_ARQUIVOS, preload=False
        )
    ...
    if torch.cuda.is_available():
        if cpu_count <= 2:
            ...
            recommended_workers = min(cpu_count - 1, 4)  # aumento para até 4 workers
    ...
    if args.num_workers is not None:
        ...
        log(f"🧠 Detectados {cpu_count} núcleos. Usando {num_workers} workers.")

    # Threads já foram configuradas acima
    ...
    # --- LOG INICIAL ---
    log("="*80)
    log("🚀 RIGELSLM v1.2.0 – TREINO OTIMIZADO PARA COLAB")
    log(f"📅 {datetime.now()}")
    log(f"💻 Dispositivo: {DISPOSITIVO}")
    log(f"📁 Dados: {PASTA_DADOS}")
    log(f"📊 Batch: {BATCH_SIZE}, Seq: {SEQ_LEN}")
    log(f"🧵 Workers: {num_workers}, Threads: {threads}")
    ...
    log(f"⚡ Pré-carregamento: {'ATIVADO' if args.preload else 'DESATIVADO'}")
    log(f"⚡ torch.compile: {'ATIVADO' if args.compile else 'DESATIVADO'}")
    log(f"⚡ Gradient checkpointing: {'ATIVADO' if args.checkpointing else 'DESATIVADO'}")
    if args.usar_registro:
    ...
    # --- MODELO ---
    model = RigelSLM(use_checkpointing=args.checkpointing).to(DISPOSITIVO)

    # Aplica torch.compile se solicitado
    if args.compile:
        try:
            model = torch.compile(model, mode="reduce-overhead")
            log("✅ torch.compile aplicado (modo reduce-overhead)")
        except Exception as e:
            log(f"⚠️ torch.compile falhou: {e}. Continuando sem compilação.", "WARNING")
    ...
    if checkpoint_disponivel and not RESUME:
        try:
            ...
            if args.no_interactive:
                # Não-interativo: continua do checkpoint (não trava nem perde progresso)
                log("ℹ️ Modo não-interativo: continuando do checkpoint automaticamente.", "INFO")
                RESUME = True
            else:
            ...
            log(f"⚠️ Checkpoint corrompido: {e}. Iniciando do zero.", "WARNING")
            RESUME = False
    ...
    while True:
        ...
        train_dataset = StreamingTextDataset(
            PASTA_DADOS, tokenizer, SEQ_LEN, VALIDAR, shuffle=True,
            max_arquivos=MAX_ARQUIVOS, preload=args.preload
        )
        train_dataset._arquivos = train_arquivos

        val_dataset = StreamingTextDataset(
            PASTA_DADOS, tokenizer, SEQ_LEN, VALIDAR, shuffle=False,
            max_arquivos=MAX_ARQUIVOS, preload=False  # validação nunca pré-carrega (economiza RAM)
        )
        val_dataset._arquivos = val_arquivos

        pin_mem = torch.cuda.is_available()
        # ================= CORREÇÃO APLICADA AQUI =================
        # Só define multiprocessing_context se num_workers > 0
        if num_workers > 0:
        ...
        if start_epoch >= EPOCHS:
            ...
            if args.no_interactive:
                log("ℹ️ Modo não-interativo: encerrando (use --epochs maior para continuar).", "INFO")
                break
