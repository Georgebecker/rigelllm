...
def _processar_parquet(caminho: str):
            ...
            if "messages" in row and isinstance(row.get("messages"), list):
                ...
                for i in range(len(msgs) - 1):
                    ...
                    if not isinstance(a, dict) or not isinstance(b, dict):
                        continue
                    if b.get("role") in ("assistant", "gpt", "modelo") and a.get("role") in ("user", "human", "humano", "system"):
...
def _linhas_parquet(caminho: str) -> int:
    """Número de linhas de um .parquet (barato: só lê o metadado)."""
    try:
        import pyarrow.parquet as pq
...
def criar_tokenizer() -> bool:
    ...
    for arq in arquivos[:5000]:
        if arq.lower().endswith('.parquet'):
            for item in _processar_parquet(arq):
        ...
        for caminho in self._listar_arquivos():
            try:
                if caminho.lower().endswith('.parquet'):
                    ...
                    itens = (processar_arquivo(caminho, validar_dialogos=self.validar_dialogos),)
                for item in itens:
                    if item is None:
                        continue
